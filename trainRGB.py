#train_example_code:CUDA_VISIBLE_DEVICES=0 python trainRGB.py --config examples/example/config4096RGB.json -n test -pm checkpoints/JournalMask/1024/iter_600000.pth.tar
#eval_example_code:CUDA_VISIBLE_DEVICES=0 python trainRGB.py --config examples/example/config4096RGB.json -n test -pm checkpoints/JournalMask/1024/iter_600000.pth.tar -p checkpoints/JournalRGB/4096/iter_1500000.pth.tar --test
import os
import argparse
from models.AutoEncoderRGB_Journal import AutoEncoder
from models.AutoEncoderMask_Journal import AutoEncoder as MaskAutoEncoder
...
def testKodak(step):
    with torch.no_grad():
        ...
        for batch_idx, (masked_input,_,image,_,image_with_alpha)in enumerate(test_loader,1):
            ...
            clipped_recon_mask = torch.clamp(clipped_recon_mask, 0,1)
            clipped_recon_mask = torch.round(clipped_recon_mask * 255)/ 255
            clipped_recon_mask = constraint(clipped_recon_mask)

            clipped_recon_image, mse_loss, bpp,bpp_feature, bpp_z= net(masked_input,mask,clipped_recon_mask,me1,me2,me3,me4)
            clipped_recon_image = torch.clamp(clipped_recon_image, 0,1)
            ...
            torchvision.utils.save_image(img, "outputKodak/"+str(cnt+1)+"img.png",alpha=True)
